...
const CardCountry = (props) => {
  ...
  {props.continent ? <h3 className="card-title">{country.continent.name}</h3>
    ...
    <div className="card-body text-primary">
      ...
      {props.continent && country.languages.length > 0 ? <ul className="card-title"><span className="">Languages:</span> {country.languages.map(len => <li className="text-muted" key={len.name}>{len.name}</li>)}</ul>
